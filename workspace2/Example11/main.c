...
static void vEnviaDados(void *pvParameters);
static void vRecebeDados(void *pvParameters);
static void vSite(void *pvParameters);
...
int main(void) {
	...
	vPrintString("projetoEC020\r\n"); //Precisa printar se não da erro no BasicIO
	...
	xQueue = xQueueCreate(10, sizeof(Dados));

	if (xQueue != NULL ) {
		xTaskCreate(vEnviaDados, "ENVIA", 240, NULL, 1, NULL);
		xTaskCreate(vSite, "SITE", 240, NULL, 1, NULL);
		xTaskCreate(vRecebeDados, "RECEBE", 240, NULL, 2, NULL);
...
void InsertDynamicValues(Dados t) {
